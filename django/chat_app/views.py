...
def add_friend(request):
    ...
    if request.GET.get("id"):
        ...
        if get_create:
            messages.add_message(
                request,
                messages.SUCCESS,
                f"{user_2.username} has been added to your friend list!",
            )
        else:
            messages.add_message(
                request,
                messages.SUCCESS,
                f"{user_2.username} is already in your friend list!",
            )
    ...
    return render(request, "chat/add_friend.html", {"all_user": all_user})
...
def start_chat(request, room_name):
    ...
    if check_user.exists():
        ...
        return HttpResponse("You do not have permission to chat with this user.")
